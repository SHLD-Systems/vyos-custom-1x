...
## Files touched (7+2 knobs) — `git diff origin/rolling..HEAD --stat` 536 insertions, 44 deletions (as of latest)

### 1. `interface-definitions/load-balancing_wan.xml.in` (+59 +26 for ping knobs, 2 comments)
...
- New per-test `test ping-count` `u32:1-50` default `3` (`numeric --range 1-50`) — tunable `-c` for `health_ping_host_metrics`; nexthop-only path keeps function default `3`.
- New per-test `test ping-interval` float `0.1-5` default `1` (`numeric --range 0.1-5`, `format txt`) — tunable `-i` frequency seconds between probes; `iputils-ping -i` default 1s, super-user <2ms allowed but clamped 0.1s min. Function default `1.0` via `interval` param, `f'{interval:g}'` rendering.
...
### 3. `python/vyos/wanloadbalance.py` (+174 +20 for ping knobs, reworked `wlb_weight_interfaces`)
...
- New `health_ping_host_metrics(host,ifname,count=3,wait_time=5,interval=1.0)` — now tunable `-c`/`-i`; clamps `count 1-50`/`interval 0.1-5`, builds `ping -c {count} -W {wait_time} -i {interval:g} -I {ifname} {host}`; 3×1s default gives loss granularity 0/0.33/0.66/1.0; success=`rc==0 and loss<1.0`; nexthop-only stays default without config.
- Retained legacy `health_ping_host` (1 probe) for compat + `health_ping_host_ttl`.

### 4. `src/helpers/vyos-load-balancer.py` (+212 +12 for ping knobs)
...
- `health_check` extended: preserves boolean `ACTIVE/FAILED` counting, but collects SLA metrics:
  ...
  - No `test` → metrics ping against `nexthop` (dhcp resolved) default `count=3 interval=1.0` via function defaults.
  - With `test` list: `ping` uses metrics with per-test `ping_count`/`ping_interval` (defaults 3/1.0, parsed `int`/`float` from `test_conf`), `ttl`/`user-defined` boolean only; after loop updates `state['sla_*']` or inits `factor 1.0` if only non-ping tests.
...
### 5. `src/conf_mode/load-balancing_wan.py` (+22 +22 for ping knobs)
...
- Added per-test `ping_count 1-50` int and `ping_interval 0.1-5` float checks (per `test_id`) with `ConfigError` mirroring XML `--range`.
...
- No-op migration for version `4→5`: existing configs valid, new `interval`/`sla.*` + `ping-count`/`ping-interval` optional with defaults; placeholder `pass` with comment.

## Update 2026-09-06 — per-test ping tuning

Extends `iputils-ping -i` frequency exposure per request: default `1s` keeps prior behavior (`-i 1` ≈ existing implicit 1s gap), `0.1s` (≈100ms >2ms super-user limit) allows faster loss granularity without changing daemon loop `interval=5s`. `count` up to `50` (loss steps 2%) vs default `3` (33% steps). No nexthop-only logic change — both knobs fallback via function signature defaults when `test` absent. Validate interplay: `count*interval + count*W` should stay < daemon `interval` to avoid overrun.
